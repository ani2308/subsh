...
if(!mysqli_connect_error()){
    $query="SELECT * FROM `".$USERTABLENAME."` WHERE `verified`=1 AND `approved`=0 AND `formStatus1`=1 AND `formStatus2`=1 AND `formStatus3`=1 AND `formStatus4`=1 AND 'UserType' = 0 AND `userID`=".$_GET["userID"];
...
if(!mysqli_connect_error()){
    if(array_key_exists("markAsApproved", $_POST) AND $_POST["userIDToApprove"]){
        $queryToApprove="UPDATE `".$USERTABLENAME."` SET `approved`=1, `approverID`=".$_SESSION['approverID']." WHERE `verified`=1 AND `approved`=0 AND 'UserType' = 0 AND `userID`=".$_POST["userIDToApprove"];
