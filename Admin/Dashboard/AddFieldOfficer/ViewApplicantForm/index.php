...
// =======================TO MARK USER AS FIELD OFFICER================================
if(!mysqli_connect_error()){
    if(array_key_exists("markAsFieldOfficer", $_POST) AND $_POST["userIDToMakeFieldOfficer"]){

        $queryToUpdate="UPDATE `".$USERTABLENAME."` SET `userLevel`=2 WHERE `userID`=".$_POST["userIDToMakeFieldOfficer"];
        if(mysqli_query($link,$queryToUpdate)){
            // echo "Success!";
            header("Location: ../");
        }else{
            // echo "Error updating!";
            ...
            <div class="card" style="padding: 10px; margin-bottom: 10px">
                <form method="POST">
                    <div class="col-lg-12" style="text-align: center;">
                        <input type="hidden" name="userIDToMakeFieldOfficer" value="<?php echo $_GET["userID"];?>">
                        <input style="padding-left: 30px;padding-right: 30px;font-weight: bold; " name="markAsFieldOfficer" type="submit" class="btn btn-success btn-lg" value="Make Field Officer">
                    </div>
                </form>
            </div>
        </div>
