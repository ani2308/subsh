...
if(!mysqli_connect_error()){
    $query="SELECT * FROM `".$USERTABLENAME."` WHERE `isOldUser`=1 AND `userID`=".$_GET["userID"];
...
if(!mysqli_connect_error()){
    if(array_key_exists("markAsVerified", $_POST) AND $_POST["userIDToVerify"]){
        $queryToVerify="UPDATE `".$USERTABLENAME."` SET `isOldUser`=2, `adminID`=".$_SESSION['adminID']." WHERE `userID`=".$_POST["userIDToVerify"];
...
<body>
	...
	<div class="alert alert-primary" role="alert" style="margin: 10px;">
	  <h4><u>Verify Old Applicants</u></h4> 
            ...
            <div class="card" style="padding: 10px; margin-bottom: 10px">
                <form method="POST">
                    <div class="col-lg-12" style="text-align: center;">
                        ...
                        <input style="padding-left: 30px;padding-right: 30px;font-weight: bold; " name="markAsVerified" type="submit" class="btn btn-success btn-lg" value="Save Changes">
                    </div>
                </form>
            </div>
        </div>
