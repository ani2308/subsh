...
<body>
	...
	<div class="alert alert-primary" role="alert" style="margin: 10px;">
	  <h4><u>Manage Farmers Under Field Officer</u></h4>
	  <b><u>List Of Field Officers</u></b>
	</div>
	...
	<table border="2px solid black" style="background: white;margin-left: 10px;">
		<tr>
			...
			<th style="padding: 10px;text-align: center;font-size: 20px;"><span class="badge badge-warning" style="padding: 10px">View FO Details</span></th>
			<th style="padding: 10px;text-align: center;font-size: 20px;"><span class="badge badge-success" style="padding: 10px">Add Farmers Under FO</span></th>
			<th style="padding: 10px;text-align: center;font-size: 20px;"><span class="badge badge-info" style="padding: 10px">View Farmers Under FO</span></th>
		</tr>
		...
		if(!mysqli_connect_error()){
			...
			$total_pages_sql = "SELECT COUNT(*) FROM `".$USERTABLENAME."` WHERE `userLevel`=2 AND `verified`=1 AND `approved`=1 AND `formStatus1`=1 AND `formStatus2`=1 AND `formStatus3`=1 AND `formStatus4`=1".$searchQuerySQL;
	        $result = mysqli_query($link,$total_pages_sql);
	        $total_rows = mysqli_fetch_array($result)[0];
	        $total_pages = ceil($total_rows / $no_of_records_per_page);

			$query="SELECT `userID`,`name`,`contactNumber`,`verifierID`,`approverID` FROM `".$USERTABLENAME."` WHERE `userLevel`=2 AND `verified`=1 AND `approved`=1 AND `formStatus1`=1 AND `formStatus2`=1 AND `formStatus3`=1 AND `formStatus4`=1".$searchQuerySQL." LIMIT $offset, $no_of_records_per_page";
					...
					<tr>
						...
						<td style="text-align: center; padding: 10px;">
							<form method="GET" action="./ViewApplicantForm">
								...
								<button style="margin-top: 2px;" class="btn btn-warning" type="submit" value="'.$row['userID'].'"><b>View FO Details</button>
							</form>
						</td>

						<td style="text-align: center; padding: 10px;">
							<form method="GET" action="./AddFarmersUnderFO">
								<input type="hidden" name="userID" value="'.$row['userID'].'">
								<input type="hidden" name="name" value="'.$row['name'].'">
								<input type="hidden" name="contactNumber" value="'.$row['contactNumber'].'">
								<button style="margin-top: 2px;" class="btn btn-success" type="submit" value="'.$row['userID'].'"><b>Add Farmers Under FO</button>
							</form>
						</td>

						<td style="text-align: center; padding: 10px;">
							<form method="GET" action="./ViewFarmersUnderFO">
								<input type="hidden" name="userID" value="'.$row['userID'].'">
								<input type="hidden" name="name" value="'.$row['name'].'">
								<input type="hidden" name="contactNumber" value="'.$row['contactNumber'].'">
								<button style="margin-top: 2px;" class="btn btn-info" type="submit" value="'.$row['userID'].'"><b>View Farmers Under FO</button>
							</form>
						</td>
