...
if(!mysqli_connect_error()){
    if(array_key_exists("AddFarmerToFO", $_POST)){
        $queryToApprove="UPDATE `".$USERTABLENAME."` SET `managerID`=".$_GET['userID']." WHERE `userID`=".$_POST["userID"];
...
<body>
	...
	<div class="alert alert-primary" role="alert" style="margin: 10px;">
	  <h4><u>Add Farmers Under Field Officer</u></h4>
	...
	<div class="alert alert-success" role="alert" style="margin: 10px;">
	  ...
	  <b>Click "Add" button to add the farmer under the selected FO.</b>
	</div>
	...
	<table border="2px solid black" style="background: white;margin-left: 10px;">
		<tr>
			...
			<th style="padding: 10px;text-align: center;font-size: 20px;"><span class="badge badge-danger" style="padding: 10px">View Farmer Details</span></th>
			<th style="padding: 10px;text-align: center;font-size: 20px;"><span class="badge badge-success" style="padding: 10px">ADD</span></th>
		</tr>
		...
		if(!mysqli_connect_error()){
			...
			$total_pages_sql = "SELECT COUNT(*) FROM `".$USERTABLENAME."` WHERE `userLevel`=1 AND `managerID`=0 AND `verified`=1 AND `approved`=1 AND `formStatus1`=1 AND `formStatus2`=1 AND `formStatus3`=1 AND `formStatus4`=1".$searchQuerySQL;
	        $result = mysqli_query($link,$total_pages_sql);
	        $total_rows = mysqli_fetch_array($result)[0];
	        $total_pages = ceil($total_rows / $no_of_records_per_page);

			$query="SELECT `userID`,`name`,`contactNumber`,`verifierID`,`approverID` FROM `".$USERTABLENAME."` WHERE `userLevel`=1 AND `managerID`=0 AND `verified`=1 AND `approved`=1 AND `formStatus1`=1 AND `formStatus2`=1 AND `formStatus3`=1 AND `formStatus4`=1".$searchQuerySQL." LIMIT $offset, $no_of_records_per_page";
				...
				echo '
					<tr>
						...
						<td style="text-align: center; padding: 10px;">
							<form method="POST">
								<input type="hidden" name="userID" value="'.$row['userID'].'">
								<input style="margin-top: 2px;" class="btn btn-success" type="submit" name="AddFarmerToFO" value="ADD">
							</form>
						</td>
