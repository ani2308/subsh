...
<body>
	...
	<table border="2px solid black" style="background: white; margin-left: 10px;">
		...
		if(!mysqli_connect_error()){
			...
			$query="SELECT `userID`,`name`,`contactNumber` FROM `".$USERTABLENAME."` WHERE `UserType` =0 AND `verified`=0 AND `approved`=0 AND `formStatus1`=1 AND `formStatus2`=1 AND `formStatus3`=1 AND `formStatus4`=1".$searchQuerySQL." LIMIT $offset, $no_of_records_per_page";
