	...
	if(!mysqli_connect_error()){
		if(array_key_exists("username", $_POST) AND array_key_exists("password", $_POST)){
			if(!($_POST["username"]=="" OR $_POST["password"]=="")){
				...
				$query="SELECT * FROM `".$ADMINTABLENAME."` WHERE `adminUsername`='".mysqli_real_escape_string($link,$_POST['username'])."'"." AND `adminPasswordHash`='".md5(mysqli_real_escape_string($link,$_POST['password']))."'";
