	...
	if(!mysqli_connect_error()){
		if(array_key_exists("username", $_POST) AND array_key_exists("password", $_POST)){
			if(!($_POST["username"]=="" OR $_POST["password"]=="")){
				...
				$query="SELECT * FROM `".$VERIFIERTABLENAME."` WHERE `verifierUsername`='".mysqli_real_escape_string($link,$_POST['username'])."'"." AND `verifierPasswordHash`='".md5(mysqli_real_escape_string($link,$_POST['password']))."'";
				...
				if(mysqli_num_rows($result)>0){//USER EXISTS WITH GIVE USERNAME AND PASSWORD
					...
					$_SESSION['verifierID']=$row['verifierID'];
					$_SESSION['verifierUsername']=$row['verifierUsername'];
...
<div class="wrapper fadeInDown">
  <div id="formContent">
    ...
    <h1 style="margin: 10px;"><u>Verifier login</u></h1>
