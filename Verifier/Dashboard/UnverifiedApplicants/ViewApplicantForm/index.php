...
if(!mysqli_connect_error()){
    $query="SELECT * FROM `".$USERTABLENAME."` WHERE `verified`=0 AND `formStatus1`=1 AND `formStatus3`=1 AND `userID`=".$_GET["userID"];
...
if(!mysqli_connect_error()){
    if(array_key_exists("markAsVerified", $_POST) AND $_POST["userIDToVerify"]){
        $queryToVerify="UPDATE `".$USERTABLENAME."` SET `verified`=1, `verifierID`=".$_SESSION['verifierID']." WHERE `verified`=0 AND `userID`=".$_POST["userIDToVerify"];
            ...
            <div class="card" style="padding: 10px; margin-bottom: 10px">
                <form method="POST">
                    <div class="col-lg-12" style="text-align: center;">
                        ...
                        <input style="padding-left: 30px;padding-right: 30px;font-weight: bold; " name="markAsVerified" type="submit" class="btn btn-success btn-lg" value="Mark As Verified">
                    </div>
                </form>
            </div>
        </div>
