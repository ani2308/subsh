...
<body>
	...
	<div class="alert alert-primary" role="alert" style="margin: 10px;">
	  <h4><u>Un-verified Applicants</u></h4>
	</div>
	...
	<table border="2px solid black" style="background: white; margin-left: 10px;">
		...
		if(!mysqli_connect_error()){
			...
			$total_pages_sql = "SELECT COUNT(*) FROM `".$USERTABLENAME."` WHERE `verified`=0 AND `approved`=0 AND `formStatus1`=1 AND `formStatus2`=1 AND `formStatus3`=1 AND `formStatus4`=1".$searchQuerySQL;
	        $result = mysqli_query($link,$total_pages_sql);
	        $total_rows = mysqli_fetch_array($result)[0];
	        $total_pages = ceil($total_rows / $no_of_records_per_page);

			$query="SELECT `userID`,`name`,`contactNumber` FROM `".$USERTABLENAME."` WHERE `verified`=0 AND `approved`=0 AND `formStatus1`=1 AND `formStatus2`=1 AND `formStatus3`=1 AND `formStatus4`=1".$searchQuerySQL." LIMIT $offset, $no_of_records_per_page";
