...
if(!mysqli_connect_error()){
    $query="SELECT * FROM `".$USERTABLENAME."` WHERE `verified`=1 AND `formStatus1`=1 AND `formStatus2`=1 AND `formStatus3`=1 AND `formStatus4`=1 AND `userID`=".$_GET["userID"];
...
// =======================TO MARK USER AS UN-VERIFIED================================
if(!mysqli_connect_error()){
    if(array_key_exists("markAsVerified", $_POST) AND $_POST["userIDToVerify"]){
        $queryToVerify="UPDATE `".$USERTABLENAME."` SET `verified`=0, `verifierID`=".$_SESSION['verifierID']." WHERE `verified`=1 AND `userID`=".$_POST["userIDToVerify"];
            ...
            <div class="card" style="padding: 10px; margin-bottom: 10px">
                <form method="POST">
                    <div class="col-lg-12" style="text-align: center;">
                        ...
                        <input style="padding-left: 30px;padding-right: 30px;font-weight: bold; " name="markAsVerified" type="submit" class="btn btn-danger btn-lg" value="Mark As Un-Verified">
                    </div>
                </form>
            </div>
        </div>
