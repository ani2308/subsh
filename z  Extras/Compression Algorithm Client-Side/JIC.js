var jic = {
        ...
        upload: function(compressed_img_obj, upload_url, file_input_name, filename, successCallback, errorCallback, duringCallback, customHeaders){
            ...
            var fdImg1=['--' + boundary, 'Content-Disposition: form-data; name="' + file_input_name + '"; filename="' + filename + '"', 'Content-Type: ' + type, '', atob(data),'--' + boundary];
            var fdImg2=['Content-Disposition: form-data; name="file2"; filename="2' + filename + '"', 'Content-Type: ' + type, '', atob(data), '--' + boundary + '--'];
            
            var fdMerged = fdImg1.concat(fdImg2);

            var formData=fdMerged.join('\r\n');
		  
            // formData=formData+['--' + boundary, 'Content-Disposition: form-data; name="file2"; filename="2' + filename + '"', 'Content-Type: ' + type, '', atob(data), '--' + boundary + '--'].join('\r\n');
